...
fn dump_rust_actuals() {
    ...
    for path in &paths {
        ...
        for k in [
            "cache_key", "compressed", "compressed_bodies", "compressed_tokens",
            "compression_ratio", "language", "language_confidence", "original",
            "original_tokens", "preserved_imports", "preserved_signatures",
            "symbol_scores", "syntax_valid",
        ] {
        ...
        if !keys.is_empty() {
            println!("MISMATCH {name} {keys:?}");
        }
        out_map.insert(name.clone(), actual.clone());
    ...
    fs::write("/tmp/rust_actuals.json", serde_json::to_string(&out_map).unwrap()).unwrap();
