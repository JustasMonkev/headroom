...
**[Serena](https://github.com/oraios/serena)** is an opt-in code-memory MCP for semantic, symbol-level code navigation. Pass `--code-memory serena` to run it on demand via `uvx` and index the current project locally. The default is `--code-memory none`, so no Git-sourced Serena code runs implicitly.

> **Upgrading from tokensave?** Earlier releases registered a `tokensave` MCP server (a downloaded Rust binary). tokensave has been retired. On your next `headroom wrap` / `headroom unwrap`, Headroom removes the `tokensave` MCP entry it installed; use `--code-memory serena` if you want Serena instead. The leftover `tokensave` binary in `~/.local/bin` and any `.tokensave/` folders are unused and safe to delete.
...
| `--offline` / `HEADROOM_OFFLINE` | `false` | Disable Headroom's **auxiliary** egress (telemetry, update checks, license reporting, model downloads). Does **not** block provider forwarding — see below. |
...
Headroom runs locally and **never sends your prompts to a Headroom-hosted service** — there
is no Headroom cloud in the request path.

It is still a proxy, though: after compressing a request it forwards that request to the
upstream you configured. If that upstream is Anthropic, OpenAI, Qwen, or any other remote
API, your prompt reaches that provider. Traffic stays on the machine only when the upstream
is itself local (Ollama, vLLM, LM Studio, …).

`--offline` / `HEADROOM_OFFLINE=1` therefore scopes to Headroom's **auxiliary** egress:

| Gated by `HEADROOM_OFFLINE` | Not gated |
|---|---|
| Telemetry beacon | Provider forwarding (`/v1/messages`, `/v1/chat/completions`, …) |
| Update / release check | Explicitly enabled exporters (OpenTelemetry, Langfuse) |
| License / usage reporting | Remote memory or vector backends you configure |
| HuggingFace model downloads (`HF_HUB_OFFLINE=1`, `TRANSFORMERS_OFFLINE=1`) | |

<Callout type="warn" title="Offline mode is not an egress firewall">
Setting `HEADROOM_OFFLINE=1` while pointing Headroom at a remote provider still sends
prompts to that provider. For a true air gap, combine it with a local upstream
(`--openai-api-url http://127.0.0.1:8000`, see [Local LLM prefill](/docs/local-llm-prefill))
or with a network-level egress block.
</Callout>
