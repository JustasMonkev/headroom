...
# Closing delimiters for the two open-ended patterns above. Both scan a
# wildcard run looking for a terminator, so on text that opens many times and
# never closes — a truncated JSON tool result, minified assets, a log full of
# "{" — the engine restarts the scan at every opener and the cost goes
# quadratic (measured: 80KB of unclosed "{" costs ~1s, 40KB of unclosed "<!--"
# ~0.7s, both scaling 4x per 2x of input). Since every match must END with
...
def detect_waste_signals(text: str, tokenizer: Tokenizer) -> WasteSignals:
    ...
    html_matches = HTML_TAG_PATTERN.findall(text) + HTML_COMMENT_PATTERN.findall(
