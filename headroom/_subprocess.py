...
def proc_identity(pid: int) -> tuple[str, float] | None:
    ...
    Used to defeat PID reuse: state tied to a PID is only trusted while the
    live PID is *the same process* that recorded it. Returns ``None`` when the
    start time can't be determined (e.g. macOS without psutil), in which case
    callers fall back to existence-only liveness — no regression, just no
    reuse protection there.

    The ``source`` tag ("psutil" vs "proc") guards against comparing values in
    different units; only like-for-like comparisons are meaningful.
    """
    try:
