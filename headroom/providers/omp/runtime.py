...
OWNERS_SUFFIX = _ownership.OWNERS_SUFFIX
...
def owners_path(models_file: Path) -> Path:
    ...
    return _ownership.owners_path(models_file)
...
def hold_models_override(
    ...
    Every session registers, isolated or shared: the owner list is what lets a
    concurrent run's exit hand the file over instead of restoring it out from
    under a peer. Only an isolated run pairs this with
    :func:`release_models_override`, since its port dies with the run; a shared
    session keeps the durable write-and-leave contract.
    """
...
def release_models_override(proxy_probe: _ownership.ProxyProbe | None = None) -> str:
