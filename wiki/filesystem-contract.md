...
| `HEADROOM_SHARED_WORKSPACE_DIR` | `= HEADROOM_WORKSPACE_DIR` | Persistent, cross-run state that must never land in a throwaway per-run workspace (managed binaries, Copilot auth, MCP install ledger, license cache, learned verbosity profile) | Read-write |
